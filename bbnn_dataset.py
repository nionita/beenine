...
class BBNNDataset(IterableDataset):
    def __init__(self, data_dir: str, rezult_target: bool = False, evaluate: bool = False, skip: int = 0):
        super(BBNNDataset).__init__()

        self.rezult_target = rezult_target
        ...
        if self.evaluate:
            self.feat_files = [os.path.join(data_dir, "file.txt")]  # Assuming a single file for evaluation
            self.targ_files = []
        else:
    ...
    def filenames(self, data_dir: str, pat: str) -> list:
        return [os.path.join(data_dir, file_name) for file_name in glob.glob(pat, root_dir=data_dir)]
...
class FileIterator:
    def __init__(self, feature_files: list, target_files: list, rezult_target: bool, evaluate: bool, skip: int, wid: int, wnum: int):
